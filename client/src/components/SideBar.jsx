...
const Sidebar = ( {isMenuOpen, setIsMenuOpen}) => {
  const { chats, setSelectedChat, theme, setTheme, user, navigate,token,createNewChat,setChats,fetchUserChats,setToken,axios } =
    useAppContext();
  ...
  const logout= () => {
    ...
    setToken(null);
    toast.success('succcefully logout')
  }
  ...
  return (
    <div className={`flex flex-col h-screen min-w-72 p-5 dark:bg-gradient-to-b from-[#242124]/30 to-[#000000]/30 border-r border-[#80609F]/30 backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-50 ${isMenuOpen ? "max-md:translate-x-0" : "max-md:-translate-x-full"}`}>
      <img
      ...
      >
        ...
        {user && <img onClick={logout} src={assets.logout_icon} className="h-5 cusror-pointer hidden not-dark:invert group-hover:block"></img>}
      </div>
